          ...
          <div class="flex justify-between items-center pb-2 border-b border-gray-200">
            <div class="flex items-center">
              ...
              <div class="cursor-pointer flex items-center">
                <Icon @click="pinChat(null, message.client.id)" icon="bi:pin-angle" width="26" height="26" class="mr-4 cursor-pointer text-black"/>
              </div>
              <div v-if="pinedChat && pinedChat.is_pinned" class="cursor-pointer flex items-center">
                <Icon @click="unPinChat(null, pinedChat.id)" icon="bi:pin-angle-fill" width="26" height="26" class="mr-4 cursor-pointer text-black"/>
              </div>
<!--              <div class="text-lg text-gray-800 font-semibold">Клиент на этапе: </div>-->
<!--              <span class="ml-2">{{ translateStatus(client.status) }}</span>-->
            ...
            <div class="flex items-center gap-5">
              ...
              />
              <span class="text-xs text-gray-400">Сегодня: {{ new Date().toLocaleDateString('ru-RU') }}</span>
            </div>
          </div>
          ...
          <div v-if="!message.is_close" class="flex items-center">
            ...
            <div v-if="!newMessagePhoto.photo_path" class="mr-4 cursor-pointer text-black"> <Icon @click="openTemplates" icon="icon-park-twotone:text-message" width="30" height="30" /></div>

            <div v-if="showTemplates" class="absolute bottom-20 left-[158px] z-20 bg-white border rounded-lg shadow-lg animate-fade-in">
            ...
            <button
              @click="sendMessages"
              class="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-r-md transition-colors duration-200 cursor-pointer"
            ...
            <div class="relative group">
              <div class="sm:ms-6 sm:flex">
                ...
                <Icon v-if="newMessage || newMessagePhoto.photo_path" @click="clearInput" class="cursor-pointer absolute bottom-[-1rem] right-[8rem] text-black" icon="fluent:text-clear-formatting-32-light" width="32" height="32" />
              </div>
            </div>
          </div>
...
export default {
  components: { PinChatsInChatModal, FileInput, Alert, ModalShowOrderScreenshot, ConsultationService, TemplateService, Icon},
    props: {
        isActive: {
            type: Boolean,
            default: false,
